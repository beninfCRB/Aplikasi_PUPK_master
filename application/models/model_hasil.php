...
class Model_hasil extends CI_Model
{
    ...
    {
        $query = "SELECT id_jwb,nama_token,nrp,nama_mhs,nama_matkul,nama_lab,folder,jawaban,hasil.TimeStamp AS time FROM hasil INNER JOIN mahasiswa ON mhs_id = id_mhs INNER JOIN lab ON lab_id = id_lab INNER JOIN matkul ON matkul_id = id_matkul ORDER BY id_jwb DESC";
        return $this->db->query($query)->result_array();
    }

    public function gethasilid($kode)
    {
        $query = "SELECT * FROM hasil INNER JOIN mahasiswa ON mhs_id = id_mhs INNER JOIN lab ON lab_id = id_lab WHERE id_jwb = $kode";
    ...
    public function storehasil()
    {
        ...
        if (!$this->upload->do_upload('jwb')) {
            ...
            if ($data2 != NULL) {
                ...
                if (isset($nrp['nrp']) == $this->input->post('nrp')) {
    ...
    {
        return $this->db->get_where('mahasiswa', ['nrp' => $keyword])->row_array();
    }
    ...
    function get_submatkul($id)
    {
        $hasil = $this->db->query("SELECT * FROM matkul WHERE smt_id= $id ")->result_array();
        return $hasil;
    }
}
